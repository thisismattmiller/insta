// Windows 95 flying logos screensaver
(function() {
    const canvas = document.getElementById('starfield');
    const ctx = canvas.getContext('2d');

    let width, height, centerX, centerY;
    const logos = [];
    const numLogos = 50;
    const speed = 0.004;
    const targetFPS = 12; // Low framerate for retro feel
    let lastFrame = 0;

    // Windows logo colors
    const colors = [
        'rgb(255, 20, 2)',    // Red
        'rgb(212, 212, 212)', // Light gray
        'rgb(39, 195, 80)',   // Green
        'rgb(40, 125, 113)',  // Teal
        'rgb(125, 134, 19)',  // Olive/Yellow
        'rgb(158, 19, 133)',  // Purple/Magenta
    ];

    // Pre-render colored logos
    const logoImages = {};
    const logoSVG = `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill="COLOR" d="M5.712 1.596l-.756.068-.238.55.734-.017zm1.39.927l-.978.137-.326.807.96-.12.345-.824zM4.89 3.535l-.72.05-.24.567.721-.017zm3.724.309l-1.287.068-.394.96 1.27-.052zm1.87.566l-1.579.069-.566 1.357 1.596-.088.548-1.338zm-4.188.037l-.977.153-.343.806.976-.12zm6.144.668l-1.87.135-.637 1.527 1.87-.154zm2.925.219c-.11 0-.222 0-.334.002l-.767 1.85c1.394-.03 2.52.089 3.373.38l-1.748 4.201c-.955-.304-2.082-.444-3.36-.394l-.54 1.305a8.762 8.762 0 0 1 3.365.396l-1.663 4.014c-1.257-.27-2.382-.395-3.387-.344l-.782 1.887c3.363-.446 6.348.822 9.009 3.773L24 9.23c-2.325-2.575-5.2-3.88-8.637-3.896zm-.644.002l-2.024.12-.687 1.68 2.025-.19zm-10.603.05l-.719.036-.224.566h.703l.24-.601zm3.69.397l-1.287.069-.395.959 1.27-.05zM5.54 6.3l-.994.154-.344.807.98-.121zm4.137.066l-1.58.069L7.53 7.77l1.596-.085.55-1.32zm1.955.688l-1.87.135-.636 1.527 1.887-.154zm2.282.19l-2.01.136-.7 1.682 2.04-.19.67-1.63zm-10.57.066l-.739.035-.238.564h.72l.257-.6zm3.705.293l-1.303.085-.394.96 1.287-.034zm11.839.255a6.718 6.718 0 0 1 2.777 1.717l-1.75 4.237c-.617-.584-1.15-.961-1.611-1.149l-1.201-.498zM4.733 8.22l-.976.154-.344.807.961-.12.36-.841zm4.186 0l-1.594.052-.549 1.354L8.37 9.54zm1.957.668L8.99 9.04l-.619 1.508 1.87-.135.636-1.527zm2.247.275l-2.007.12-.703 1.665 2.042-.156zM2.52 9.267l-.718.033-.24.549.718-.016zm3.725.273l-1.289.07-.41.96 1.287-.03.412-1zm1.87.6l-1.596.05-.55 1.356 1.598-.084.547-1.322zm-4.186.037l-.979.136-.324.805.96-.119zm6.14.633l-1.87.154-.653 1.527 1.906-.154zm2.267.275l-2.026.12-.686 1.663 2.025-.172zm-10.569.031l-.739.037-.238.565.72-.016zm3.673.362l-1.289.068-.41.978 1.305-.05zm-2.285.533l-.976.154-.326.805.96-.12.342-.84zm4.153.07l-1.596.066-.565 1.356 1.612-.084zm1.957.666l-1.889.154-.617 1.526 1.886-.15zm2.28.223l-2.025.12-.685 1.665 2.041-.172.67-1.613zm-10.584.05l-.738.053L0 13.64l.72-.02.24-.6zm3.705.31l-1.285.07-.395.976 1.287-.05.393-.997zm11.923.07c1.08.29 2.024.821 2.814 1.613l-1.715 4.183c-.892-.754-1.82-1.32-2.814-1.664l1.715-4.133zm-10.036.515L4.956 14l-.549 1.32 1.578-.066.567-1.338zm-4.184.014l-.996.156-.309.79.961-.106zm6.14.67l-1.904.154-.617 1.527 1.89-.154.632-1.527zm2.231.324l-2.025.123-.686 1.682 2.026-.174zm-6.863.328l-1.3.068-.397.98 1.285-.054zm1.871.584l-1.578.068-.566 1.334 1.595-.064zm1.953.701l-1.867.137-.635 1.51 1.87-.137zm2.23.31l-2.005.122-.703 1.68 2.04-.19.67-1.61z"/></svg>`;

    function createLogoImage(color) {
        return new Promise((resolve) => {
            const svg = logoSVG.replace('COLOR', color);
            const blob = new Blob([svg], { type: 'image/svg+xml' });
            const url = URL.createObjectURL(blob);
            const img = new Image();
            img.onload = () => {
                URL.revokeObjectURL(url);
                resolve(img);
            };
            img.src = url;
        });
    }

    function resize() {
        width = canvas.width = window.innerWidth;
        height = canvas.height = window.innerHeight;
        centerX = width / 2;
        centerY = height / 2;
    }

    function createLogo() {
        return {
            x: (Math.random() - 0.5) * width * 2,
            y: (Math.random() - 0.5) * height * 2,
            z: Math.random() * 1500 + 500,
            color: colors[Math.floor(Math.random() * colors.length)]
        };
    }

    async function init() {
        resize();

        // Pre-render all colored logos
        for (const color of colors) {
            logoImages[color] = await createLogoImage(color);
        }

        for (let i = 0; i < numLogos; i++) {
            logos.push(createLogo());
        }

        // Clear canvas first
        ctx.fillStyle = 'black';
        ctx.fillRect(0, 0, width, height);

        draw();
    }

    function draw(timestamp) {
        // Limit framerate for retro choppiness
        if (timestamp - lastFrame < 1000 / targetFPS) {
            requestAnimationFrame(draw);
            return;
        }
        lastFrame = timestamp;

        // Clear completely (no smooth fade - more retro)
        ctx.fillStyle = 'black';
        ctx.fillRect(0, 0, width, height);

        // Sort by z for proper depth
        logos.sort((a, b) => b.z - a.z);

        for (let i = 0; i < logos.length; i++) {
            const logo = logos[i];

            logo.z -= speed * 1000;

            if (logo.z <= 1) {
                logo.x = (Math.random() - 0.5) * width * 2;
                logo.y = (Math.random() - 0.5) * height * 2;
                logo.z = 1500;
                logo.color = colors[Math.floor(Math.random() * colors.length)];
            }

            // Project 3D to 2D
            const scale = 400 / logo.z;
            const sx = logo.x * scale + centerX;
            const sy = logo.y * scale + centerY;
            const size = Math.max(8, 100 * scale);

            // Skip if outside screen
            if (sx < -size || sx > width + size || sy < -size || sy > height + size) continue;

            // Draw the Windows logo (snap to pixels for retro feel)
            const img = logoImages[logo.color];
            if (img) {
                ctx.drawImage(img, Math.floor(sx - size/2), Math.floor(sy - size/2), Math.floor(size), Math.floor(size));
            }
        }

        requestAnimationFrame(draw);
    }

    window.addEventListener('resize', resize);
    init();
})();
